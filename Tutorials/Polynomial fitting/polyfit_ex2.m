...
%                     fitting using CaΣoS, while imposing some shape
...
%           specifically, that it must be a nonnegative function and
%           strictly nondecreasing over the domain of interest. 
%
...
% define sos problem
sos.x = [p; s1];
sos.g = [-s1*(-x^2+4*x)+nabla(p,x);  % monotonicity constraint
         p];                         % nonnegativity constraint
sos.f = theta'*theta; 
