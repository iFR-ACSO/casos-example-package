...
%                    hierarchy in CaSOS. We minimize a univariate polynomial
%                    over a compact basic semialgebraic set.
%
%  Problem: Minimize f(x) = x^4 + 10x over K = {x | 0.5 - x^2 >= 0}
%
...
% support constraint: u(x) = 0.5 - x^2 >= 0 defines compact set K
u = 0.5 - x'*x;
...
% 2) support constraint: supp(mu) in K  (implemented via mu.support(u))
...
g_meas = [primalize(mu.support(u))];    % support constraint
