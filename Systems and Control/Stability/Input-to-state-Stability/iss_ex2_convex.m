...
%                     on the method from [1]. A simple satellite model is 
%                     used to find a valid ISS Lyapunov function.
%
%  Date:    15.12.2025
%
%  Reference:
...
% get a simple controller for both attitude (MRP) and angular rates
Kd = diag([3;7;10]);
...
solveTimeStart = tic;
sol = S();
solveTimeStop = toc(solveTimeStart);


if strcmp('SOLVER_RET_SUCCESS',S.stats.UNIFIED_RETURN_STATUS)
  
    fprintf('Successfully solved!\n')
    fprintf('Solver was built in %f seconds and it took %f seconds to solve.',[buildTimeStop,solveTimeStop])
    % remove small coefficients and rescale
    remove_coeffs(subs(sol.x,x,Dx*x),sqrt(eps))
